...
meaning every spoken word transcribed exactly. On real support calls you almost
never have that. What you have is a **human-written summary**: cleaned,
condensed, maybe 3 lines per minute. Run WER against a summary and *both*
engines score as catastrophically wrong, not because they're bad, but because
the reference isn't verbatim. WER is measuring the wrong thing.
...
| **Entity accuracy** | no | Does the engine get the domain terms and proper nouns right? Reported as **exact** and **fuzzy** (fuzzy credits near-miss spellings). This is the metric that survives a condensed reference, and the one that matters for downstream RAG and search. |
...
so we know every word), which lets us show WER alongside the reference-light
metrics and demonstrate that the reference-light ones track WER closely. On real
condensed references you'd drop WER and keep the other two.

---

## Results (synthetic, 4 calls, ~4 min)

| Engine | WER (lower is better) | Entity exact | Entity fuzzy | Phrase recall | Wall clock |
|--------|------:|-------------:|-------------:|--------------:|-----------:|
...
- **Cloud vs local is a latency and accuracy trade**, not just accuracy.
  Deepgram was roughly 55x faster wall-clock here (cloud vs CPU `small`) and a
  bit more accurate. A local GPU, or a smaller and faster model, changes that
  calculus.
- **Exact vs fuzzy entity accuracy is the honest way to report term errors.**
  Deepgram's exact 89.8% rises to fuzzy 97.8%: most "misses" are near-miss
  spellings of brand and jargon terms, not dropped content. Reporting only
  exact-match overstates the problem; reporting only fuzzy hides it. Show both.
- The reference-light metrics (entity, phrase recall) **rank the engines the
  same way WER does**, which is evidence they're usable when WER isn't
  available.
...
- **Full-file diarization on mono phone audio tends to collapse to one
  ...
  count pinned to 2 recovers a credible agent and client split.
- **The ASR model size barely affects diarization.** Speaker clustering is done
  by the diarizer (for example pyannote), which is independent of the Whisper
  model. A bigger ASR model improves the *text*, not *who-spoke-when*.
...
# 1. Generate the synthetic multi-call recording and verbatim ground truth
...
- **Synthetic is not real.** TTS audio is clean; real telephony has codecs,
  overlap, crosstalk, and noise that hurt both engines more, and unevenly.
- **Single language and domain** (Spanish software support).
